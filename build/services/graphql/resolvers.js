...
var _default = app => {
  const lookups = {
    nuts: val => id => id.length === [null, 2, 3, 5, 8][val] && val < 5 && id !== 'DG',
    parent: val => id => id.startsWith(val)
  };
  const genesApiService = app.service('genesapi');
  const elasticsearch = app.get('elasticsearch'); // console.log(
  ...
  const regions = async (obj, args, context, info) => {
    ...
    const ids = idAggregation.aggregations.ids.buckets.map(bucket => bucket.key); // FIXME
